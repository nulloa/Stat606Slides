...
out2.alccpu <- laGP.R(Xref, 6, 50, X, Z, d=out2$mle$d, method="alc",parallel="omp")
@

<<exTimes, echo=TRUE>>=
## compare times
xtable(data.frame(nn=out$time, alc=out2$time, alcCPU=out2.alccpu$time,
...
\begin{frame}[fragile]
<<explot, echo=FALSE, fig.height=5, fig.width=5>>=
...
\begin{frame}[fragile]
<<exfunct2, echo=TRUE, eval=FALSE>>=
laGP.R(Xref, 6, 50, X, Z, d=0.1, method="alc",parallel="gpu")#alc.gpu = TRUE)
@
\end{frame}
