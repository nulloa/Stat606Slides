...
\item The basic idea is to split our sample space ($D$) into $M$ blocks where $\bigcup_k D_k = D$ and $D_k \bigcap D_l = \emptyset$
\end{itemize}
\end{frame}
...
out <- laGP(Xref, 6, 500, X, Z, method="nn")
## second and pass via ALC, MSPE, and ALC-ray respectively
out2 <- laGP(Xref, 6, 500, X, Z, d=out$mle$d, method="alc", alc.gpu=TRUE)
out2.alccpu <- laGP.R(Xref, 6, 500, X, Z, d=out2$mle$d, method="alc", parallel="none")
out2.alcgpu <- laGP.R(Xref, 6, 500, X, Z, d=out2$mle$d, method="alc", parallel="gpu")
@
...
text(X[out$Xi,], labels=1:500, cex=0.6)
text(X[out2$Xi,], labels=1:500, cex=0.6, col="red")
