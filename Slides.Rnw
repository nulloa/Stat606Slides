...
\begin{frame}
<<SimStudyplot2, echo=FALSE, fig.width=7, fig.height=4>>=
load("SimRes.RData")
dat <- melt(SimRes)
sub2 <- subset(dat,Var2=="truepred"|Var2=="nnPred"|Var2=="alcPred")
sub2$Type <- factor(sub2$Var2)
levels(sub2$Type) <- c("True", "NN", "ALC")
ggplot(data=sub2, aes(x=Var1, y=value, color=Type)) + geom_line() + labs(x="Theta", y="Prediction")
@
\end{frame}
...
\begin{frame}
\begin{itemize}
  \item Clearly, ALC is as good as NN for prediction
